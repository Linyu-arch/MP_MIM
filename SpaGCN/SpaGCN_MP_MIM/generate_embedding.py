...
def generate_spagcn_output(anndata, pca, res,dim, img_path,pca_opt,sample,cluster_num):            #add MP and kmeans
    ...
    clf = SpaGCN()
    clf.set_l(l)
    clf.train(anndata, adj, barcode_list, num_pcs=pca,dim = dim, init_spa=True, init="kmeans", res=res, tol=5e-3,pca_opt = pca_opt, sample=sample, n_clusters=cluster_num)        #add MP and kmeans
