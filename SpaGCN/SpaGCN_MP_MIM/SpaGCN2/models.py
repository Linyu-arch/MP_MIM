...
class simple_GC_DEC(nn.Module):
    ...
    def fit(self, X,adj, barcode_list,lr=0.001, max_epochs=5000, update_interval=5, trajectory_interval=50,weight_decay=5e-4,opt="sgd",init="louvain",n_neighbors=10,res=0.4,n_clusters=10,init_spa=True,tol=1e-3,sample='151507'):
        ...
        print("MP_MIM_SpaGCN. Start Time: %s seconds" %
          (start_time))
        #MP_parameter_set
        MP_k_num_list = [10,20,30,40,50,60,70,80,90]
        ...
        for MP_k_num_in in MP_k_num_list:
            for MP_l_num_in in MP_l_num_list:
                ...
                adj_features_np = generate_adj_nx_weighted_adj(features_np, distanceType='euclidean', k=MP_k_num_in)
                adj_self_loop = preprocess_graph_self_loop(adj_features_np)
                ...
                emb_name = str(count)+'_spagcn_default_raw_gat_ave_MP_k'+str(MP_k_num_in)+'_l'+str(MP_l_num_in)
                emb_name_list.append(emb_name)
                ####MI_max
        ...
        MIrow_max_pd = pd.DataFrame({'Name':emb_name_list,'MIrow_max':embedding_MIrow_max_list})
        MIrow_max_pd.to_csv(embedding_in_SpaGCN_folder+sample+'_spagcn_k10_to_k90_MIrow_max_result.csv')
        ...
        embedding_name = embedding_in_SpaGCN_folder+sample+'_'+emb_name_list[int(count_set)-1]+'_embedding.csv'
        print(embedding_name)
        #graph_embedding_df = pd.read_csv(embedding_name[0])
        graph_embedding_df = pd.read_csv(embedding_name)
