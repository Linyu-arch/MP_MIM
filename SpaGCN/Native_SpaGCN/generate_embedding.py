...
def generate_spagcn_output(anndata, pca, res,dim, img_path,pca_opt,cluster_num):    # add kmeans
    ...
    clf = SpaGCN()
    clf.set_l(l)
    clf.train(anndata, adj, num_pcs=pca,dim = dim, init_spa=True, init="kmeans", res=res, tol=5e-3,pca_opt = pca_opt, n_clusters=cluster_num)     ##add kmeans
